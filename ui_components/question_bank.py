    ...
    if isinstance(questions, list) and questions:
        for idx, question in enumerate(questions, 1):
            with st.container(border=True):
                ...
                with col1:
                    st.write(f"**Q{idx}:** {str(question)}")
                with col2:
                    marks = topic.get("typical_marks", "N/A")
                    st.metric("Marks", marks)

                if st.checkbox(f"Show answer outline for Q{idx}", key=f"q_{selected_topic}_{idx}"):
                    st.markdown("""
                    <div style='background-color: #f0f7ff; padding: 15px; border-left: 4px solid #667eea; border-radius: 5px; margin-top: 10px;'>
                    <b>Answer Outline:</b>
                    <ul>
                    <li>Review lecture notes for this topic</li>
                    <li>Check similar PYQ answers from previous years</li>
                    <li>Practice writing key points clearly</li>
                    </ul>
                    </div>
